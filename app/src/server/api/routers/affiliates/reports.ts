...
export const getCreativeReport = publicProcedure
	...
	.query(
		async ({ ctx, input: { from, to, merchant_id, banner_id, type } }) => {
			let type_filter = {};
			...
			const merchant_ids =
				await ctx.prisma.merchants_creative_stats.groupBy({
...
export const getLandingPageData = publicProcedure
	...
	.query(
		async ({
			ctx,
			input: { from, to, merchant_id, url, creative_type },
		}) => {
	...
	.query(
		async ({
			...
			if (filter === "ftd" || filter === "totalftd") {
				trader_report_resource =
					await ctx.prisma.reporttraders.findMany({
						orderBy: {
	...
	.query(async ({ ctx, input: { from, to, merchant_id, search_type } }) => {
		...
		dateRanges.forEach((date) => {
			(async () => {
				...
				if (!from && !to) {
					where = `sb.unixRdate BETWEEN ${getUnixTime(
						from
					)} AND ${getUnixTime(to)} AND `;
				}
				...
				for (let i = 0; i <= Object.keys(merchant_ww).length; i++) {
					if (merchant_ww[i]?.id) {
						where = `sb.merchant_id=${
							merchant_ww[i]?.id ?? 0
						} AND `;
					}
				}

				const arrClicksAndImpressions =
					await ctx.prisma.traffic.groupBy({
						by: ["affiliate_id"],
						where: {
							type: "traffic",
							views: 1,
				...
				for (
					let i = 0;
					i < Object.keys(arrClicksAndImpressions).length;
					i++
				) {
				...
				for (let i = 0; i < Object.keys(regww).length; i++) {
					...
					const strAffDealType =
						await ctx.prisma.affiliates_deals.findMany({
							orderBy: {
								id: "desc",
							},
							where: {
					...
					for (
						let j = 0;
						j < Object.keys(strAffDealType).length;
						j++
					) {
						if (
					...
					if (regww[i]?.type === "real") {
						...
						if (!boolTierCplCount) {
							...
							if (
								...
								).includes(regww[i]?.affiliate_id)
							) {
								arrTierCplCountCommissionParams["amount"]++;
							} else {
								arrTemp["merchant_id"] = regww[i]?.merchant_id;
								arrTemp["affiliate_id"] =
				...
				for (let i = 0; i < Object.keys(sales_ww).length; i++) {
					if (
						earliestTimeForNetRev >
						(sales_ww[i]?.rdate || new Date())
					) {
						earliestTimeForNetRev =
							sales_ww[i]?.rdate ?? new Date();
					}
				...
				if (displayForex) {
					...
					for (
						let i = 0;
						i < Object.keys(traderStats_2).length;
						i++
					) {
...
export const getSubAffiliateReport = publicProcedure
	...
	.query(async ({ ctx, input: { from, to } }) => {
		...
		while (_index < Object.keys(affiliate_ww).length) {
			...
			const arrClicksAndImpressions =
				await ctx.prisma.sub_stats.aggregate({
					where: {
